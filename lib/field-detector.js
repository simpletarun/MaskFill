...
(function () {
  ...
  function hostMatches(hostname, rule) {
    ...
    var r = String(rule || '').trim().toLowerCase().replace(/:.*$/, ''); // drop any port
    if (!r) return false;
    if (hostname.slice(0, 4) === 'www.') hostname = hostname.slice(4);
    if (r.slice(0, 4) === 'www.') r = r.slice(4);
    if (!r) return false;
    return hostname === r || hostname.slice(-(r.length + 1)) === '.' + r; // rule 'bank.com' also covers 'www.bank.com', 'login.bank.com'
  }

  // per-site skip rule: "bank.com | [name=userid]"  → CSS selector;
  // "bank.com | promo" → substring match on the field's name/id/class/placeholder/label
  function siteRuleMatches(el, rule, hostname) {
    try {
      var parts = String(rule || '').split('|');
      if (parts.length < 2 || !hostMatches(hostname, parts[0])) return false;
      var needle = String(parts[1] || '').trim();
      if (!needle) return false;
      if (needle.charAt(0) === '#' || needle.charAt(0) === '.' || needle.charAt(0) === '[') {
        try { return !!el.matches && el.matches(needle); } catch (e) { return false; }
      }
      var joined = sanitize([
      ...
      var sn = sanitize(needle);
      return (sn && joined.indexOf(sn) !== -1) || joined.indexOf(needle.toLowerCase()) !== -1;
  ...
  function isIgnorable(el, ignorePatterns, hiddenOk) {
    ...
    if (!hiddenOk && typeof getComputedStyle === 'function' && !isChoice) {
      try {
  ...
  var FieldDetector = {
    ...
    siteRuleMatches: siteRuleMatches
  };
