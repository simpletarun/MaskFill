...
(function () {
  ...
  // fields that regenerate cheaply and can safely be kept non-repeating.
  // personaLock'd types (names/email/phone/city/country/...) repeat-guard via the persona ring.
  var NO_REPEAT_TYPES = ['middleName', 'nickname', 'jobTitle', 'department', 'industry', 'company',
    'companyWebsite', 'bio', 'sentence', 'paragraph', 'words', 'text', 'search', 'degree', 'university',
    'school', 'securityAnswer', 'timezone', 'addressLine2', 'buildingNumber', 'apartment', 'filename',
    'fileExt', 'domain', 'subdomain', 'url', 'mimeType', 'companyId', 'orderId', 'invoiceId', 'productId',
    'userId', 'otp', 'pin', 'passport', 'driversLicense', 'taxId', 'ssn', 'mac', 'ip', 'ipv6', 'port', 'cidr'];
  ...
  var RandomData = {
    ...
    getPersona: function () {
      ...
      function build() {
        var fn, ln, latFn, latLn, hiNames = false;
        if (_locale === 'hi') hiNames = true;
        else if (_locale === 'en_hi') hiNames = Math.random() < 0.5;
        if (hiNames) {
      ...
      var candidate = build();
      var seen = {};
      for (var i = 0; i < _recentPersonas.length; i++) seen[_recentPersonas[i]] = true;
      // ponytail: recent-persona ring keeps repeat fills from echoing the same identity;
      // registry capped at 40, rebuild-and-skip up to 25 tries
      var attempts = 0;
      var key = candidate.email + '|' + candidate.firstName + '|' + candidate.lastName + '|' + candidate.username;
      while ((candidate.email === '' || seen[key]) && attempts < 25) {
        candidate = build();
        key = candidate.email + '|' + candidate.firstName + '|' + candidate.lastName + '|' + candidate.username;
        attempts++;
      }
      _persona = candidate;
      _recentPersonas.push(key);
      if (_recentPersonas.length > 40) _recentPersonas.shift();
      return _persona;
    },
    ...
    generate: function (type, opts) {
      ...
      // ponytail: per-type recent-value ring stops random text fields from repeating
      // across fills; skipped for persona-locked and tiny-pool types. Cap 300, 6 retries.
      if (typeof v === 'string' && v && NO_REPEAT_TYPES.indexOf(type) !== -1) {
        var list = (_recentValues[type] = _recentValues[type] || []);
        var tries = 0;
        while (list.indexOf(v) !== -1 && tries < 6) {
          v = String(run.call(this));
          tries++;
        }
        list.push(v);
