...
(function () {
  ...
  // merge keys that would clobber the prototype chain (stored settings are attacker-adjacent)
  var SKIP_KEYS = { __proto__: 1, constructor: 1, prototype: 1 };
  ...
  function deepMerge(base, over) {
    ...
    if (typeof base === 'object' && typeof over === 'object') {
      ...
      Object.keys(over).forEach(function (k) {
        if (SKIP_KEYS[k]) return;
        if (out[k] === undefined && base[k] === undefined) out[k] = over[k];
      });
      return out;
    }
    return over;
  ...
  function normalize(s) {
    ...
    if (!Array.isArray(out.siteRules)) out.siteRules = [];
    out.siteRules = out.siteRules.filter(function (r) { return typeof r === 'string' && r && r.indexOf('|') !== -1; });
  ...
  var GlobalData = {
    ...
    saveSettings: function (s) {
      return new Promise(function (resolve) {
        chrome.storage.local.set({ settings: normalize(s) }, function () { resolve(); });
      });
    }
  };
