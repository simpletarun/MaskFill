...
(function () {
  ...
  function syncValueTracker(el, value) {
  ...
  var FormFiller = {
    setNativeValue: function (el, value, silent) {
      ...
      syncValueTracker(el, value == null ? '' : value);
      if (!silent) {
        dispatch(el, 'input');
        dispatch(el, 'change');
        dispatch(el, 'blur');
    ...
    fillCheckbox: function (el, settings) {
      var checked = false;
      if (settings && settings.checkAllBoxes) {
        checked = true;
      } else if (settings && settings.autoCheckTerms) {
        try {
      ...
      try {
        // real click toggles the box and reaches React/Vue/Angular listeners; bare
        // checked=true + synthetic events can be reverted by framework controlled state
        if (el.checked !== checked) el.click();
      } catch (e) {}
      if (el.checked !== checked) {
        // click() missing/suppressed (preventDefault / fake element) → force the state
        el.checked = checked;
        try {
